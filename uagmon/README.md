...
- **등록 주소 가드** — 루프백·링크로컬(클라우드 메타데이터)·우회표기(10진/16진/8진·
  IPv4-mapped IPv6)는 등록을 거부합니다. **공개 IP(IPv4·IPv6 대칭)는 기본 차단**하고 사내
  사설망(RFC1918, IPv6 ULA `fc00::/7`)만 허용합니다(v2.257 — 사내 모니터가 대상이므로).
- **연결 직전 SSRF 재검증**(v2.257) — 대상이 호스트네임이면 **접속 직전 DNS 해석 결과를 다시
  가드로 검증**하고, 접속은 해석된 IP 로만 하되 TLS SNI·Host 헤더는 원래 호스트로 핀합니다
  (DNS 가 나중에 루프백/공개서버로 바뀌는 리바인딩·저장 후 host 바꿔치기 차단). 저장 시점 검증과
  연결 시점 검증의 **이중 방어**입니다.
- **host 변경 시 비밀번호 무효화**(v2.257) — 등록의 host 를 바꾸면 저장된 비밀번호를 이월하지 않고
  재입력을 요구합니다(공개 서버로 host 만 바꿔치기해 저장 자격증명을 빼내는 경로 차단).
...
기본은 사내 사설망 전용입니다. 공개 대상을 모니터해야 하면 환경변수로 엽니다:

| 변수 | 뜻 |
|---|---|
| `UAGMON_ALLOW_PUBLIC=true` | 공개 IP(IPv4/IPv6) 등록·연결 허용 |
| `UAGMON_ALLOWED_DOMAINS=corp.local,dvc.internal` | 지정한 도메인 접미사의 호스트네임만 허용(미설정 시 전 호스트명 허용) |
| `UAGMON_DATA=<dir>` | 데이터 디렉터리(기본 `<앱>/data`) |
