...
/** 웹에 표시되는 모든 'DataCenter 선택' 목록의 순서를 지정한다(↑/↓로 정렬 후 저장). vCenter 순서 지정과 동일 패턴. */
function DatacenterOrderCard() {
  ...
  const sortName = () => setList([...list].sort((a, b) => String(a.name || a.id).localeCompare(String(b.name || b.id), 'ko', { numeric: true })));

  return (
    <div className="card" style={{ marginBottom: 22, padding: '12px 14px' }}>
      <div className="flex between wrap" style={{ alignItems: 'center' }}>
        <b style={{ fontSize: 14 }}>DataCenter 표시 순서 ({list.length})</b>
        <button className="logout-btn" style={{ padding: '7px 12px' }} onClick={() => setOpen((o) => !o)}>{open ? '접기' : '순서 지정'}</button>
      </div>
      <div className="muted" style={{ fontSize: 12, marginTop: 4 }}>여기서 정한 순서가 웹의 모든 “DataCenter 선택” 콤보박스·목록에 동일하게 적용됩니다.</div>
      {open && (
        <>
