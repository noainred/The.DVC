      ...
      {t && (
        <>
          ...
          <div className="kpis" style={{ marginBottom: 12 }}>
            <div className="card kpi" style={{ textAlign: 'center' }}><div className="label">전체 VM</div><div className="value">{fmt(t.count)}</div><div className="meta">구동중 {fmt(t.poweredOn)} · 정지 {fmt(t.poweredOff)}</div></div>
            <div className="card kpi" style={{ textAlign: 'center' }}><div className="label">할당 vCPU / vCore</div><div className="value" style={{ color: 'var(--accent)' }}>{fmt(t.vcpu)}</div><div className="meta">vCPU {fmt(t.vcpu)} · vCore {fmt(t.vcpu)}</div></div>
            <div className="card kpi" style={{ textAlign: 'center' }}><div className="label">평균 CPU 사용량</div><div className="value">{t.avgCpuUsagePct}%</div><div className="meta">구동중 VM 기준</div></div>
            <div className="card kpi" style={{ textAlign: 'center' }}><div className="label">할당 메모리 합계</div><div className="value" style={{ color: 'var(--purple)' }}>{fmt(t.ramGB)}<small> GB</small></div><div className="meta">≈ {(t.ramGB / 1024).toFixed(1)} TB</div></div>
            <div className="card kpi" style={{ textAlign: 'center' }}><div className="label">평균 메모리 사용률</div><div className="value">{t.avgMemUsagePct}%</div><div className="meta">구동중 VM 기준</div></div>
            <div className="card kpi" style={{ textAlign: 'center' }}><div className="label">할당 디스크 합계</div><div className="value" style={{ color: 'var(--accent-2)' }}>{fmt(t.diskTB)}<small> TB</small></div><div className="meta">{fmt(t.diskGB)} GB</div></div>
            <div className="card kpi" style={{ textAlign: 'center' }}><div className="label">평균 디스크 사용률</div><div className="value">{t.avgDiskUsagePct ?? 0}%</div><div className="meta">프로비저닝 대비 사용</div></div>
            <div className="card kpi" role="button" tabIndex={0}
              style={{ cursor: 'pointer', textAlign: 'center', outline: (gpuOnly || gpuType) ? '1px solid var(--green)' : 'none' }}
