...
/**
 * @param drops  서버 collectDrops — [{ id, agent, at, tries }] (최신이 앞)
...
export function collectDropNote(drops, nameOf = (id) => id, now = Date.now()) {
  ...
  return `'지금 수집' 요청 **${recent.length}건**이 결과 없이 폐기됐습니다(마지막 ${ago(now - Number(recent[0].at))}) — `
    + `엣지${agents.length ? `(${agents.join(', ')})` : ''}가 요청을 가져갔지만 새 수집 결과가 오지 않아 한 번 더 요청한 뒤 버렸습니다: `
    + `${names.join(', ')}${more}. 수집이 되었다는 뜻이 아닙니다 — 특수기능 › 엣지 로그에서 그 장비의 수집 오류를 확인하세요.`;
}
