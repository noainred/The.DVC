...
export const TOOLS = [
  ...
  { k: 'bm-usage', icon: '📟', label: '베어메탈 사용률', desc: '서버 분석 › 구분 › Baremetal(미가상화 물리) 로 분류된 서버만 CPU·메모리·디스크·네트워크·HBA 사용률을 수집한다 — 법인 단위로 켜고, OS 계정(베어메탈 스토리지 등록부)이 있으면 다섯 지표 전부, iDRAC 텔레메트리만 있으면 CPU·메모리·I/O. 원시 표본과 하루 단위 롤업(평균·최대)을 함께 보관해 장기 추세를 본다', aka: ['bmusage', '베어메탈 사용률', 'baremetal usage', '물리서버 사용률', 'CPU 사용률', 'HBA 사용률', '미가상화'], adminOnly: false },
  { k: 'link-check', icon: '🔗', label: '통신 점검', desc: '중앙 포탈과 엣지 포탈 사이의 모든 통신을 단계별(DNS → TCP → TLS → HTTP → 인증 → 정체)로 재서 어디서 막혔는지 기록한다 — 점검 대상은 등록된 수집 서버·vCenter에서 자동으로 뽑고, 중앙이 잴 수 없는 방향(엣지→중앙 push·설정 pull·엣지→vCenter·엣지↔엣지)은 엣지가 재서 올린다. 실패와 상태 변화는 인증서 만료일·응답 조각까지 남겨 이슈 분석 자료로 쓴다', aka: ['linkcheck', '통신 점검', '통신 테스트', 'link check', '엣지 통신', '중앙 엣지 통신', '연결 점검', '도달성', 'reachability', '엣지간 통신'], adminOnly: true },
  { k: 'comm-map', icon: '🕸️', label: '통신 지도', desc: '중앙 포탈과 엣지(수집 서버)의 통신을 라디얼 그래프로 본다 — 가운데 중앙, 안쪽 링에 엣지(상태색), 바깥 링에 각 엣지가 위임받은 vCenter·스토리지·SAN·PDU. 허브↔엣지 선은 중앙→엣지 pull 과 엣지→중앙 push 두 가닥이고 마지막 두 조회 사이에 관측된 통신에만 점이 흐른다. 노드를 누르면 사유·조치·수신 통계·통신 점검 결과가 나온다', aka: ['commmap', '통신 지도', '통신 시각화', '엣지 지도', 'comm map', 'topology', '토폴로지', '중앙 엣지 그래프', '라디얼'], adminOnly: true },
  { k: 'data-flow', icon: '🔀', label: '데이터 흐름 지도', desc: '포탈 사이를 오가는 모든 경로(엣지→중앙 push·pull·작업 인출·결과 회신, 중앙→엣지 호출)를 노드 그래프로 본다 — 왼쪽 데이터 종류(인벤토리·iDRAC·GPU·전력·스토리지·SAN·장애 감시·사용자·운영 작업·등록), 가운데 경로 버스(눈금 1개 = 실제 라우터에 선언된 경로 1개), 오른쪽 엣지. 선은 기록이 있는 연결만 그리고 실패·낡음을 색으로 구분한다. 엣지 안의 수집 상태는 누를 때만 가져온다', aka: ['dataflow', '데이터 흐름', '데이터 플로우', 'data flow', 'push pull', '경로 지도', '노드 그래프'], adminOnly: true },
  { k: 'device-flow', icon: '🧭', label: '3단 지도(장비 → 엣지 → 메인)', desc: '장비·엣지·메인 포탈을 세 칸으로 본다 — 왼쪽은 엣지마다 붙은 장비를 종류별 묶음(vCenter·iDRAC·스토리지·SAN·PDU)으로, 가운데는 엣지, 오른쪽은 메인. 엣지와 메인 사이 선은 방향 넷(올림·가져감·메인이 가져옴·메인이 보냄) 중 가장 나쁜 상태로 칠하고, 묶음을 누르면 장비 목록이 펼쳐진다. 메인이 직접 수집하는 장비와 어느 엣지에도 붙일 수 없는 장비도 따로 보인다', aka: ['device flow', '3단 지도', '장비 엣지 메인', '장비-엣지-메인', 'edge main', '엣지 메인 통신', '3단계'], adminOnly: true },
  { k: 'portal-check', icon: '🧪', label: '포탈 점검', desc: '중앙 포탈과 엣지 포탈의 설정이 실제로 맞물려 있는지 점검한다 — 첫 항목은 토큰 점검: 등록된 모든 엣지·수집 서버의 토큰을 모아 중복(서로 다른 엣지가 같은 값, 수집 토큰이 중앙 토큰과 같음)을 찾고, 중앙에 저장된 값으로 실제로 통신해 보고, 엣지가 저장하고 있는 값과 같은지 확인한다. 토큰 값은 화면에 나오지 않는다(sha256 앞 8자 지문과 길이만). 둘째 인벤토리 점검(위임 vCenter push 수신 여부), 셋째 아키텍처 점검(이 서버가 자기 라우트 게이트·도구 매핑·카탈로그·엣지 로그 표·BIG_JSON·DB·설정 파일·import 그래프의 정합을 스스로 본다 — 못 읽은 항목은 정상이 아니라 확인 불가)', aka: ['portalcheck', '포탈 점검', '토큰 점검', 'token check', '토큰 중복', '토큰 불일치', '수집 토큰', 'CENTRAL_TOKEN', 'COLLECTOR_TOKEN', '엣지 토큰', '인벤토리 점검', '아키텍처 점검', 'arch check', '라우트 게이트', '정합 점검'], adminOnly: true },
  { k: 'edge-log', icon: '📜', label: '엣지 로그 · 진행상태', desc: '각 엣지(수집 서버) 포탈의 콘솔 로그와 폴러 진행상태를 중앙에서 그때그때 당겨서 본다 — 수집·push·pull 워커가 지금 무엇을 하고 있는지, 마지막 성공·실패가 언제였는지. 중앙이 엣지에 닿지 못하면 요청만 남기고 엣지가 스스로 인출해 회신한다(inbound 가 막힌 법인도 동작). 비밀번호·토큰은 엣지에서 가린 뒤 보낸다', aka: ['edgelog', '엣지 로그', 'edge log', '수집 서버 로그', '진행상태', '워커 상태'], adminOnly: true },
  { k: 'part-faults', icon: '🔧', label: '파트 장애(부품)', desc: '서버(iDRAC)·스토리지의 부품 단위 장애를 한 곳에서 — PSU·디스크·메모리(DIMM)·팬·CPU·컨트롤러·노드/SP·배터리. 상태가 바뀐 순간만 DB 에 남기고(열림/변화/해소) 발생 시 알림 발송 · 확인 불가(unknown)와 빈 슬롯(absent)을 정상에도 장애에도 넣지 않고 따로 표시 · 엣지가 현지에서 판정해 장애만 중앙으로 push', aka: ['part-fault', '부품 장애', '하드웨어 장애', 'PSU', 'DIMM', '디스크 장애', 'FRU'], adminOnly: true },
  ...
  // v2.608 — Arista CloudVision(CVP) 에 등록된 네트워크 스위치를 읽는다(엣지 위임 또는 중앙 직접). adminOnly 아님 — 조회는 tools + 전체 범위.
  { k: 'cvp', icon: '🛰️', label: 'Arista CloudVision (CVP)', aka: ['CloudVision', 'Arista', 'CVP', '아리스타'], desc: 'CVP 에 등록된 Arista 네트워크 스위치 — 인벤토리·EOS 버전·장애 파트(전원·팬·온도·트랜시버)·포트 구성·포트 사용량 추이·BGP 피어 요약 · 엣지 현지 수집→중앙 집계 또는 중앙 직접 · 토큰/ID·비밀번호 인증' },
