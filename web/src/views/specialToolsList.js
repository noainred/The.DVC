...
export const TOOLS = [
  ...
  { k: 'portal-check', icon: '🧪', label: '포탈 점검', desc: '중앙 포탈과 엣지 포탈의 설정이 실제로 맞물려 있는지 점검한다 — 첫 항목은 토큰 점검: 등록된 모든 엣지·수집 서버의 토큰을 모아 중복(서로 다른 엣지가 같은 값, 수집 토큰이 중앙 토큰과 같음)을 찾고, 중앙에 저장된 값으로 실제로 통신해 보고, 엣지가 저장하고 있는 값과 같은지 확인한다. 토큰 값은 화면에 나오지 않는다(sha256 앞 8자 지문과 길이만)', aka: ['portalcheck', '포탈 점검', '토큰 점검', 'token check', '토큰 중복', '토큰 불일치', '수집 토큰', 'CENTRAL_TOKEN', 'COLLECTOR_TOKEN', '엣지 토큰'], adminOnly: true },
