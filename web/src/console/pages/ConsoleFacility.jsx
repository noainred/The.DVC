  ...
  return (
    <>
      <div className="dvc-kpis">
        ...
        <KpiCard label="PDU" value={ps ? fmtInt(ps.devices) : '—'} accent="#e6edf6" meta={ps ? (ps.devices ? `수집 정상 ${ps.ok} · 실패 ${ps.failed} · 임계 위반 ${ps.violations}${ps.powerW ? ` · ${(ps.powerW / 1000).toFixed(1)} kW` : ''}` : '등록된 PDU 없음') : perms.pdu ? '수집 대기' : "권한 필요('tools')"} />
        <KpiCard label="호스트 온도 (ESXi/iDRAC 보고)" value={maxT != null ? `${maxT}°C` : '—'} accent={maxT == null ? '#8b9bb4' : maxT >= 26 ? '#ef4444' : maxT >= 24 ? '#f59e0b' : '#22c55e'} meta={hosts.data ? `최고값 · 측정 ${fmtInt(measured)}/${fmtInt(hostN)}대 · 26°C 이상 ${hot}대` : '호스트 수집 대기'} />
        <KpiCard label="iDRAC 연동 호스트" value={hosts.data && hostN ? fmtPct((idracN / hostN) * 100) : '—'} accent="#22d3ee" meta={hosts.data ? `${fmtInt(idracN)} / ${fmtInt(hostN)}대 (호스트 ↔ iDRAC 매핑)` : '호스트 수집 대기'} />
        <KpiCard label="BMC(iDRAC) 폴러" value={lr ? `${fmtInt(lr.ok)}/${fmtInt((lr.ok || 0) + (lr.failed || 0))}` : '—'} accent={lr ? (lr.failed ? '#f59e0b' : '#22c55e') : '#8b9bb4'} meta={polls.idrac.data ? `최근 실행 응답/전체 · 등록 ${fmtInt(polls.idrac.data.poller?.servers)}대${lr?.at ? ` · ${new Date(lr.at).toLocaleTimeString('ko-KR')}` : ''}` : perms.idrac ? '폴러 상태 대기' : '관리자 권한 필요 (/admin/idrac)'} />
      </div>
          ...
          <PollState poll={hosts}>
            {rows.length === 0 ? <Empty>범위 안에 호스트가 없습니다.</Empty> : (
              <div style={{ padding: '6px 18px 14px' }}>
                {rows.map((r) => (
                  <div key={r.vcenterId} style={{ display: 'grid', gridTemplateColumns: 'minmax(170px, 1.2fr) minmax(70px, 1fr) 70px minmax(100px, 1.4fr) 60px', gap: 12, alignItems: 'center', padding: '8px 0', borderBottom: '1px solid rgba(36,48,73,.5)' }}>
                    <div style={{ minWidth: 0 }}><div style={{ fontSize: 12.5, fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{r.name}</div><div className="dvc-cellsub" style={{ color: REGION_COLORS[r.region] || '#5d6b85', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{r.city || r.vcenterId} · {r.hosts} 호스트{r.disconnected ? ` · 끊김 ${r.disconnected}` : ''}</div></div>
                    <div style={{ display: 'flex' }}><Bar pct={maxKw ? ((r.powerKw ?? 0) / maxKw) * 100 : 0} color="#3b82f6" large /></div>
                    <div className="dvc-num" style={{ textAlign: 'right', fontSize: 11.5, color: '#fbbf24' }}>{r.powerKw != null ? `${r.powerKw} kW` : '—'}</div>
                    ...
                    <div className="dvc-num" style={{ textAlign: 'right', fontSize: 11.5, color: r.maxTemp == null ? '#5d6b85' : r.maxTemp >= 26 ? '#f87171' : r.maxTemp >= 24 ? '#fbbf24' : '#8b9bb4' }}>{r.maxTemp != null ? `${r.maxTemp}°C` : '—'}</div>
                  </div>
                ))}
                    ...
                    <tbody>
                      {rows.map((r) => (
                        <tr key={r.vcenterId}>
                          ...
                          <td className="num" style={{ color: r.hosts - r.idracBacked ? '#fbbf24' : '#5d6b85' }}>{r.hosts - r.idracBacked}</td>
                          <td className="num">{r.powerReporting}</td>
                          <td className="num" style={{ color: r.disconnected ? '#f87171' : '#5d6b85' }}>{r.disconnected}</td>
                          <td className="num" data-sort={r.maxTemp ?? ''} style={{ color: r.maxTemp == null ? '#5d6b85' : r.maxTemp >= 26 ? '#f87171' : '#8b9bb4' }}>{r.maxTemp != null ? `${r.maxTemp}°C` : '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </STable>
                    ...
                    <tbody>
                      {pduDevs.map((d) => (
                        <tr key={d.id}>
                          ...
                          <td className="num" style={{ color: '#fbbf24' }}>{d.powerW != null ? `${fmtInt(d.powerW)} W` : '—'}</td>
                          <td className="num" style={{ color: d.tempMaxC == null ? '#5d6b85' : d.tempMaxC >= 26 ? '#f87171' : '#8b9bb4' }}>{d.tempMaxC != null ? `${d.tempMaxC}°C` : '—'}</td>
                          <td className="num dvc-dim">{d.sensors}</td>
                          <td className="num" style={{ color: d.violations ? '#f87171' : '#5d6b85' }}>{d.violations}</td>
