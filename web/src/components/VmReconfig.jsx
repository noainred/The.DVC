...
function VmReconfigModal({ vm, onClose }) {
  ...
  const [adds, setAdds] = useState([]);            // [GB, ...]
  ...
  const load = () => fetchJson(`/admin/vm/${encodeURIComponent(vm.id)}/hardware`).then((r) => {
    ...
    setCpu(r.hw.cpu); setRamGB(Math.round((r.hw.memMB || 0) / 1024)); setErr(null);
  ...
  const buildPlan = () => {
    ...
    const diskAdds = adds.map((gb) => ({ sizeGB: numOr(gb, 0) })).filter((a) => a.sizeGB > 0);
  ...
  const summary = () => {
    ...
    adds.forEach((gb) => { if (numOr(gb, 0) > 0) s.push(`디스크 추가 +${numOr(gb, 0)}GB`); });
          ...
          <div className="flex gap" style={{ marginTop: 6, flexWrap: 'wrap', alignItems: 'center' }}>
            {adds.map((gb, i) => (
              <span key={i} className="flex" style={{ gap: 4, alignItems: 'center' }}>
                <input className="input" type="number" min={1} placeholder="GB" value={gb} onChange={(e) => setAdds((a) => a.map((x, j) => (j === i ? e.target.value : x)))} style={{ width: 90 }} />
                <button className="logout-btn" style={{ padding: '2px 8px' }} onClick={() => setAdds((a) => a.filter((_, j) => j !== i))}>✕</button>
              </span>
            ))}
            <button className="logout-btn" style={{ padding: '5px 10px' }} onClick={() => setAdds((a) => [...a, ''])}>+ 디스크 추가</button>
          </div>

          <div className="muted" style={{ fontSize: 12, margin: '14px 0 4px' }}>네트워크 어댑터(NIC)</div>
          {hw.nics.map((n) => (
            <label key={n.key} className="flex between" style={{ padding: '4px 0', gap: 12, alignItems: 'center' }}>
              <span style={{ fontSize: 13 }}>{n.label} <span className="muted">· {n.network || '—'} · {n.macAddress || ''}</span></span>
              <span className="flex" style={{ gap: 4, alignItems: 'center', color: 'var(--red)' }}>
