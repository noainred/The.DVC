...
describe('경로 → 작업 이름', () => {
  it('아는 경로는 사람 말로 바꾼다', () => {
    expect(taskLabel('/tools/waste')).toMatchObject({ label: 'Optimization 분석', known: true, slow: false });
  ...
  it('접두 일치는 더 구체적인 것이 이긴다', () => {
    ...
    expect(taskLabel('/tools/waste').label).toBe('Optimization 분석');
...
describe('진행 중 목록 → 표시 행', () => {
  ...
  it('오래 기다린 것부터, 같은 작업은 하나로 묶고 건수를 붙인다', () => {
    ...
    expect(out[1]).toMatchObject({ label: 'Optimization 분석', ms: 4_000, count: 1 });
