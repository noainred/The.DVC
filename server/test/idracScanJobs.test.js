...
const { enqueueIdracScan, takeIdracScanJobs, setIdracScanResult, setIdracScanProgress, getIdracScanResult, listIdracScanJobs, getIdracScanJobLog, recentPollingAgents, cancelIdracScanJob, createPushScanJob, reapClaims } = await import('../src/central/idracScanJobs.js');
...
test('claim→ack: 인출 후 ack 없으면 재인출 대기로 복귀, 한도 초과 시 오류', () => {
  const reqId = enqueueIdracScan('ClaimEdge', { ips: '10.5.0.1-5', username: 'root', password: 'pw', datacenterId: 'ce' });
  const FUTURE = Date.now() + 100 * 60_000; // claim 기한(90초) 훨씬 뒤

  // (모듈 전역 jobs를 다른 테스트와 공유하므로 전역 카운트가 아닌 '이 잡'의 상태 전이만 검증.)
  // 1차 인출(claim) → running.
  assert.equal(takeIdracScanJobs('ClaimEdge').length, 1);
  assert.equal(getIdracScanResult(reqId).state, 'running');
  // ack 없이 기한 초과 → 다시 대기로 복귀(재인출 가능).
  reapClaims(FUTURE);
  assert.equal(getIdracScanResult(reqId).state, 'pending');

  // 2차 인출 → 다시 running, 또 ack 없음 → 복귀.
  assert.equal(takeIdracScanJobs('ClaimEdge').length, 1);
  reapClaims(FUTURE);
  assert.equal(getIdracScanResult(reqId).state, 'pending');

  // 3차 인출(claims=3) → ack 없으면 이번엔 한도 초과로 오류 종결.
  assert.equal(takeIdracScanJobs('ClaimEdge').length, 1);
  reapClaims(FUTURE);
  assert.equal(getIdracScanResult(reqId).state, 'error');
});

test('claim→ack: 첫 진행 보고(ack)가 오면 재수확 대상이 아니다', () => {
  const reqId = enqueueIdracScan('AckEdge', { ips: '10.5.1.1-5', username: 'root', password: 'pw' });
  takeIdracScanJobs('AckEdge');
  setIdracScanProgress(reqId, { scanned: 1, total: 5 }); // ack
  reapClaims(Date.now() + 100 * 60_000);
  assert.equal(getIdracScanResult(reqId).state, 'running', 'ack된 잡은 재수확되지 않고 running 유지');
});
