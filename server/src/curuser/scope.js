 ...
 *  ④ 전원이 켜져 있다
 *  ⑤ 템플릿이 아니다
 *  ⑥ VMware Tools 가 **실행 중**이다(게스트 실행의 전제)
 *
...
export const SKIP_REASON = Object.freeze({
  ...
  template: '템플릿입니다.',
  'no-tools': 'VMware Tools 가 실행 중이 아닙니다 — 게스트 안에서 명령을 실행할 수 없습니다.',
  'no-creds': '이 vCenter(또는 이 VM)의 게스트 계정이 설정돼 있지 않습니다 — 설정 › GPU 사용량 수집의 게스트 계정을 함께 씁니다.',
 ...
 * @param {object}   p
 * @param {(vcId:string, vmId:string, isWin:boolean)=>boolean} p.hasCreds 게스트 계정 유무(없으면 대상 제외)
 * @returns {{ targets: object[], skipped: Array<{vmId,name,vcenterId,folder,reason}>, overLimit: number, vcenters: string[] }}
 */
export function resolveTargets(vms, settings, { hasCreds = () => true } = {}) {
  const s = settings || {};
  ...
  for (const vm of (vms || [])) {
    ...
    if (!hasCreds(vcId, vm.id, true)) { skipped.push({ ...base, reason: 'no-creds' }); continue; }
    targets.push(base);
  }
