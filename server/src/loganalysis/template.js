...
export function templateOf(msg) {
  ...
  s = s
    ...
    // v2.591 S2: 앞보기를 {0,80} 로 묶는다 — 무제한 `[\w.-]*` 는 단어 경계마다 끝까지 훑어 하이픈·점이 많은 긴 줄에서
    //   O(n²) 였다(2,000자 5.9ms · 4,000자 24.6ms — 붙여넣기 6MB 가 이벤트 루프를 24초 멈췄다). 80자 뒤에야 숫자가 나오는
    //   식별자는 사실상 없다.
    .replace(/\b(?=[\w.-]{0,80}\d)(?=[\w.-]{0,80}[A-Za-z])[A-Za-z0-9][\w.-]{2,}\b/g, '<id>') // 숫자+문자 섞인 식별자(호스트명 등)
