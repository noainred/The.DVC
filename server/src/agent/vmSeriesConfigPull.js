...
const PULL_MS = clampIntervalMs(Number(process.env.AGENT_VMSERIES_CONFIG_PULL_MS) || 10 * 60_000, 10 * 60_000, 60_000); // v2.600 EDGE2600-05
...
export async function pullVmSeriesConfigNow() {
  ...
  try {
    ...
    if (res.status === 404) {
      ...
      if (_log404(c.kind, c.reason)) console.warn(`[vmseries-pull] ${c.reason}`);
      return last;
    }
