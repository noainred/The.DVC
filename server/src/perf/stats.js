 ...
 * 정렬 비용이 든다. 버킷 카운터는 요청당 `counts[i]++` 한 번(O(1), 상수 메모리)이다.
 * 백분위는 **버킷 경계 사이 선형 보간**이라 근사값이다 — 화면에 '근사' 임을 밝힌다(추정 금지 규칙).
 *
 * ⚠️ v2.503 정직 기록 — v2.498 의 버킷은 **튜닝에 쓸 수 없었다**: 첫 경계가 50ms 라
 * **50ms 미만 요청이 전부 0번 버킷**에 들어갔다. 그러면 p50·p95·p99 가 모두 같은 버킷에서 나오고,
 * `summarizeRoute` 의 '관측 최댓값 상한' 에 걸려 **셋 다 maxMs 로 붕괴**한다. 실측(mock 11 vCenter·
 * 2,242 VM): `/datastores` p50=p95=p99=max=2ms, `/vms` 25ms, `/hosts` 13ms, `/summary` 19ms —
 * 전 라우트가 그 모양이었다. 이 서버의 API 는 스냅샷 집계라 거의 전부 50ms 미만이므로,
 * 표가 사실상 '최댓값' 열 하나를 네 번 보여주고 있었다(측정하고 있다는 착시).
 * 그래서 **1·2·5·10·25ms 경계를 추가**한다. 비용은 라우트당 카운터 5칸(400 라우트 × 8B ≈ 16KB)뿐이고
 * 핫패스는 그대로 O(1) 이다. 이 경계들을 다시 빼면 같은 붕괴가 재발한다.
 */

/**
 * 버킷 상한(ms). 마지막 버킷은 이 값 초과(∞).
 * 앞쪽 5개(1~25ms)는 '스냅샷 집계 라우트끼리의 차이' 를, 뒤쪽은 '초 단위 지연이 얼마나 잦은가' 를 본다.
 * 카운터는 메모리에만 있고 파일로 저장하지 않으므로(설정만 `perf-monitor.json`), 경계를 바꿔도
 * 마이그레이션 대상이 없다 — 프로세스 재시작 시 누계가 0 부터 다시 쌓일 뿐이다.
 */
export const BUCKETS_MS = Object.freeze([1, 2, 5, 10, 25, 50, 100, 250, 500, 1000, 2000, 3000, 5000, 10_000, 20_000, 60_000]);
