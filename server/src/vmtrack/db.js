 ...
 *   changes : 그 스냅샷에서 '생긴/사라진' VM 만 1행씩 — 클러스터·호스트·데이터스토어 포함.
...
function initSqlite() {
  ...
  return import('node:sqlite').then(({ DatabaseSync }) => {
    ...
    const st = {
      insSnap: db.prepare(`INSERT INTO snaps (slot, ts, vcenter_id, total, on_count, added, removed, baseline)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(slot, vcenter_id) DO UPDATE SET
          ts=excluded.ts, total=excluded.total, on_count=excluded.on_count,
          added=excluded.added, removed=excluded.removed, baseline=excluded.baseline`),
      ...
      series: db.prepare(`SELECT id, slot, ts, vcenter_id, total, on_count, added, removed, baseline
        FROM snaps WHERE vcenter_id=? AND ts>=? ORDER BY ts`),
      seriesAllVc: db.prepare(`SELECT id, slot, ts, vcenter_id, total, on_count, added, removed, baseline
        FROM snaps WHERE vcenter_id<>'' AND ts>=? ORDER BY ts`),
 ...
 *   perVc: [{ vcenterId, total, onCount, added:[vm...], removed:[vm...], baseline }]
 */
export async function commitSnapshot({ slot, ts, perVc, totalRow }) {
  ...
  try {
    ...
    st.insSnap.run(slot, ts, '', totalRow.total, totalRow.onCount, totalRow.added, totalRow.removed, totalRow.baseline ? 1 : 0);
    for (const vc of perVc) {
      st.insSnap.run(slot, ts, vc.vcenterId, vc.total, vc.onCount, vc.added.length, vc.removed.length, vc.baseline ? 1 : 0);
