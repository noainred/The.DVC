...
import { requireRole, requirePerm, listUsers, createUser, updateUser, deleteUser, beginTotpEnroll, confirmTotpEnroll, disableTotp, verifyUserOtp, getUser, setLocalPassword, clearLoginCredentials } from '../auth/auth.js';
...
adminRouter.put('/security/session', adminOnly, requireSettingsOwner, (req, res) => {
  ...
  let after;
  try {
    after = saveSessionSecurity({ idleLogoutEnabled: req.body?.idleLogoutEnabled, idleLogoutMin: req.body?.idleLogoutMin, settingsOwners: req.body?.settingsOwners, loginPolicy: req.body?.loginPolicy });
