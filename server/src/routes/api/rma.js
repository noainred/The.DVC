...
api.get('/tools/rma', adminOnly, (_req, res) => {
  ...
  const agents = listRmaAgents().map((g) => ({ ...g, hasPassword: !!pwMeta[g.agent.toLowerCase()]?.hasPassword, hasToken: tokens.has(g.agent.toLowerCase()) }));
  const seen = new Set(agents.map((a) => a.agent.toLowerCase()));
  // 하트비트가 없는(=RMA 미배포) 알려진 엣지도 보여 배포 대상을 고를 수 있게 한다.
  const known = knownAgentNames().filter((n) => !seen.has(String(n).toLowerCase())).map((n) => ({ agent: n, instances: [], onlineCount: 0, pending: 0, lastSeen: null, hasPassword: !!pwMeta[String(n).toLowerCase()]?.hasPassword, hasToken: tokens.has(String(n).toLowerCase()), notDeployed: true }));
  res.json({ agents: [...agents, ...known], catalog: catalog(), settings: getRmaSettings(), doneTtlMs: DONE_TTL });
});
