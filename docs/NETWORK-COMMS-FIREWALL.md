...
| TCP 22 (sshd) | SSH | 관리자 | 포탈 기능이 아니라 호스트 관리용. **v2.485 부터 설정 › Security › 호스트 접근 제어에서 열림/허용목록/완전 차단을 포탈이 firewalld 로 적용**할 수 있다([HOST-ACCESS.md](HOST-ACCESS.md)). |

> **호스트 인바운드를 포탈에서 제어(v2.485)**: 위 두 행(SSH·포탈 포트, 선택 시 80/443)과 추가 포트 규칙은 설정 › Security › **호스트 접근 제어**에서 출발지 허용목록으로 좁힐 수 있다. 엣지 에이전트·수집 에이전트·Prometheus 가 이 호스트의 4000 에 접속한다면 **그 출발지 대역을 웹 허용목록에 반드시 포함**해야 한다(허용목록은 운영자 브라우저만이 아니라 4000 으로 들어오는 모든 클라이언트에 적용된다). 적용은 런타임 → N분 내 미확정 시 자동 복원(commit-confirm).
...
| 23 | Edge/Collection Agent | Central Portal (/api/central/*) | inbound | HTTPS/HTTP | 4000 | 4000 | PORT / CENTRAL_TOKEN | 중앙 측 수신 엔드포인트 집합: /api/central/{assignment, result, inventory, idrac-scan-jobs, idrac-scan-progress, idrac-scan-result, gpu-guest-data, ping-jobs, ping-result, agent-config, log-queries, log-query-result, capture-jobs, capture-result, ip-scan-assignment, ip-scan-result, users-config, gpu-guest-config}. 사용자 인증 밖, X-Central-Token으로만 게이트(엣지별 개별 토큰이면 그 agent 데이터만 접근 가능). | server/src/routes/central.js:22-209; 마운트 server/src/index.js:83 (app.use('/api/central', centralRouter)) |
...
| 26 | Edge/Collection Agent | Central Portal | outbound | HTTPS/HTTP (GET, JSON) | CENTRAL_URL의 포트(기본 4000) | 4000 | CENTRAL_URL / CENTRAL_TOKEN | **중앙→엣지 사용자 배포(pull)**: 엣지가 GET /api/central/users-config?agent=&lt;name&gt; 으로 자기에게 배포된 포탈 접속 계정(역할·passwordHash)을 주기적으로 인출해 로컬 users.json에 반영(managedBy:'central'). 중앙이 엣지로 들어오는 인바운드가 없어 NAT/폐쇄망 안전. | server/src/agent/usersConfigPull.js; 수신측 routes/central.js /users-config |
| 27 | Edge/Collection Agent | Central Portal | outbound | HTTPS/HTTP (GET, JSON) | CENTRAL_URL의 포트(기본 4000) | 4000 | CENTRAL_URL / CENTRAL_TOKEN | **중앙→엣지 GPU 게스트 설정 배포(pull)**: 엣지가 GET /api/central/gpu-guest-config?agent=&lt;name&gt; 으로 배포된 vCenter/게스트 계정 설정을 인출해 로컬 gpu-guest.json에 적용(변경 시에만). | server/src/agent/gpuGuestConfigPull.js; 수신측 routes/central.js /gpu-guest-config |
| 28 | Central Portal | Edge Host (SSH) | outbound | SSH (port 22) | 22 | 22 | 저장된 배포 대상(SSH 계정) | **토큰 강제 동기화**: 수집 서버 연결이 403(토큰 불일치)일 때, 중앙이 SSH로 엣지 portal.env의 COLLECTOR_TOKEN을 교체하고 서비스를 재시작한다(URL 포트를 리슨 프로세스로 역추적해 해당 인스턴스만 수정). | server/src/agent/deploy.js forceCollectorToken/resolvePortalUnit; routes/admin.js /collectors/:id/force-token |
...
| 2 | Browser (guacamole-common-js / Guacamole.Client) | Portal (guacdTunnel WebSocketServer) | bidirectional | WebSocket (Guacamole protocol 텍스트 명령 스트림) — HTTP Upgrade | 4000 (포탈 HTTP 포트와 동일) | 4000 | PORT (config.js:24, 기본 4000) | 브라우저 RDP 콘솔. URL /api/remote/rdp 로 Upgrade. 접속 직전 POST /api/remote/rdp-ticket(동일 포탈 4000, admin/operator)로 자격증명→1회용 티켓 ID 발급 후, WS 쿼리엔 ?token=JWT + mappingId + ticket 만 싣는다(비밀번호 URL 미노출 — 상위 프록시 액세스 로그/브라우저 히스토리 잔존 차단; 티켓 없으면 구경로 username/password/domain 쿼리로 폴백). 포탈이 guacd 핸드셰이크(select→args→size/audio/video/image→connect→ready) 수행 후 양방향 중계 | server/src/proxy/guacdTunnel.js:45-75, server/src/routes/remote.js:27-32, server/src/proxy/rdpTicket.js, server/src/index.js:143 |
...
### ⑤ 부가 서비스(AD/LDAP · LLM · NSX · Horizon · 알림 · 업그레이드 · 메트릭) (17)
...
| 2 | Portal (server) | Active Directory / LDAP DC (TLS) | outbound | LDAPS | 636 | 636 | AD_URL(ldaps://dc:636), AD_TLS_REJECT_UNAUTHORIZED(**기본 true=검증**, false로만 opt-out) | 동일 AD 인증을 TLS로. ldaps:// URL 사용 시. 인증서 검증은 기본 ON이며 AD_TLS_REJECT_UNAUTHORIZED=false로만 해제한다(MITM 시 AD 비밀번호 탈취·역할 위조 위험). 그룹→역할 매핑은 CN/전체 DN 완전일치(AD_GROUP_MATCH). | server/src/auth/ad.js:22, 30, 71 |
...
| 15 | Prometheus / Grafana / OTel 수집기 | Portal /metrics | inbound | HTTP | 4000 | 4000 | METRICS_EXPORT_TOKEN(**미설정 시 404 비활성**), METRICS_ALLOW_ANON(무인증 공개 옵트인), METRICS_ALLOW_QUERY_TOKEN, PORT | 메트릭 익스포트 — GET /metrics가 스냅샷(vCenter up, 호스트 CPU/MEM/전력/GPU, 데이터스토어, VM 카운트)을 Prometheus 텍스트 포맷으로 노출. 기본은 Authorization: Bearer 헤더 인증(불일치 403). | server/src/routes/metricsExport.js, server/src/index.js |
| 16 | Portal (server) | VMware Horizon Connection Server | outbound | HTTPS | 443 | 443 | 설정 → 특수기능 '라이선스 만료일 확인'에서 등록(CONFIG_DIR/horizon.json 0600), HORIZON_TLS_VERIFY | 라이선스 만료일 직수집 — POST /rest/login → GET /rest/config/v1/licenses → POST /rest/logout. 결과는 10분 캐시(화면 열 때마다 왕복하지 않음), 라이선스 키는 마스킹 저장·표시. 사설 인증서 대비 기본 TLS 검증 생략(HORIZON_TLS_VERIFY=true로 강제). | server/src/horizon/horizon.js; routes/api.js /tools/license-expiry |
| 17 | Portal (server) | NSX Manager | outbound | HTTPS | 443 | 443 | 설정 → NSX 관리(nsx.json), NSX_TLS_REJECT_UNAUTHORIZED | 라이선스 만료일 수집 — GET /api/v1/licenses(만료 epoch·is_expired)를 NSX 갱신 주기마다 함께 조회해 '라이선스 만료일 확인'에 반영. | server/src/nsx/client.js licenses() |
...
- **호스트 접근 제어(v2.485)**: 포탈 호스트의 SSH/웹/추가 포트 인바운드를 설정 › Security › 호스트 접근 제어에서 firewalld 로 좁힌다. 잠금 방지: 런타임 적용 → N분 내 미확정 시 자동 복원, 웹 허용목록에 요청자 IP 필수, 적용/확정에 본인 OTP. 서비스 계정 sudoers(`/etc/sudoers.d/vmware-portal-hostaccess`)는 install.sh 가 설치하며 업그레이드로 올라온 서버는 화면 안내대로 root 가 한 번 추가. 함께 쓰면 좋은 오픈소스(fail2ban·OpenSCAP·WireGuard 등)는 [HOST-ACCESS.md](HOST-ACCESS.md) §5.
