...
**중앙 화면에서 쓰는 엣지 진단 도구** (전부 admin · 전체 범위 계정 전용)

| 도구 | 언제 쓰나 | 참고 |
|---|---|---|
| 특수기능 › **포탈 점검 › 토큰 점검** (`#/tools/portal-check`, v2.560) | 중앙에 저장된 수집 토큰과 엣지 토큰이 같은지 · 토큰 중복 · 앞뒤 공백 | 중앙이 엣지 `/api/collector/ping` 을 두드려 200 이면 일치가 증명되고, 403 이면 '다르다' 까지만 압니다. 엣지 자기보고(`GET /api/collector/token-check`)는 수집 토큰 게이트라 공유 토큰만 쓰는 엣지에서도 동작합니다 |
| 특수기능 › **포탈 점검 › 인벤토리 점검** (같은 화면, v2.570) | 위임(site) vCenter 인벤토리가 중앙에 실제로 들어오는지 | 정상 수신 / 낡음 / 수신 이력 없음 / **거부됨**(mock 차단·소유권 충돌·토큰 불일치)을 나눠 보입니다. 거부 기록의 엣지 이름은 검증되지 않은 값입니다 |
| 특수기능 › **엣지 로그 · 진행상태** (`#/tools/edge-log`, v2.549) | 엣지의 최근 로그·폴러 상태를 중앙에서 보고 싶을 때 | 누를 때만 엣지로 나갑니다(상시 수집 없음). 수집 서버와 같은 URL·수집 토큰을 써서 새 방화벽 허용이 필요 없고, 닿지 않으면 엣지가 인출해 가는 폴백 요청을 등록합니다 |
| 특수기능 › **통신 점검** (`#/tools/link-check`, v2.552~, 기본 꺼짐) | 엣지→중앙 push·설정 pull, 엣지→vCenter, 엣지↔엣지가 살아 있는지 | ⚠ 엣지 측 측정(`/api/central/link-check-config`·`/link-check`)은 **엣지별 개별 토큰 전용**입니다. 공유 `CENTRAL_TOKEN`/`EDGE_TOKEN` 만 쓰는 엣지는 403 이라 그 엣지의 링크가 '측정 없음'(시간이 지나면 '보고 없음(대기 초과)')으로 남습니다 — 3절처럼 개별 토큰을 발급하세요 |
| 설정 › 📋 Log › **로그 분석(개선점)** (v2.583) | 엣지 로그를 반출하지 않고 개선점을 뽑을 때 | 원천 '엣지 로그' 는 위 엣지 로그 화면에서 먼저 가져온 보관분을 분석합니다. 엣지 서버의 `journalctl` 출력을 '붙여넣기'(8MB · 20만 줄까지)로 넣을 수도 있습니다 |
