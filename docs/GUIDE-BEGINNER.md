...
- 로그인이 안 되면: 같은 PC에서 같은 계정으로 8회(기본값) 틀리면 15분 동안 잠기고, 시간이 지나면 자동으로
  풀립니다. 급하면 관리자에게 요청하세요(민감 작업의 OTP 재인증은 5회 실패 시 10분 잠금으로 별도입니다).
...
| 탭 | 용도 |
|---|---|
| **Overview** | 전세계 KPI(vCenter/호스트/VM/CPU/메모리/스토리지/전력/GPU) + 서버·게스트 수량(서버 합계 = 물리 전용 + 가상화 호스트, 법인별 표). 매일 첫 화면 |
...
- **🔒 회색 카드**는 내 계정에 권한이 없는 것입니다 — 필요하면 관리자에게 요청하세요.
  (관리자가 '허용 목록' 방식으로 지정한 계정은 허용된 카드만 보이고 나머지는 아예 보이지 않습니다.)

처음 써보기 좋은 도구: **Guest OS 종류/버전**(우리 환경의 OS 분포), **스냅샷 있는 VM**,
**중복 IP 찾기**, **서버 온도**(iDRAC·ESXi 수집 온도, 법인별), **일일 헬스체크 리포트**(아침 점검 요약).
...
- **'불러오는 중…' 에서 오래 멈춰요** → 몇 초가 지나면 작업 줄 아래에 **요청 ID**와 서버 상태(서버가 처리 중 / 서버는 이미 응답 / 기록 없음)가 보입니다. 그 **요청 ID**를 관리자에게 알려 주면, 관리자가 서버 기록에서 같은 요청을 찾아 어디서 늦어졌는지 확인할 수 있습니다.
