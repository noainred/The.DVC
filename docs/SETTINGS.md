...
포탈 상단 **설정** 메뉴의 33개 화면을 화면 순서대로 설명한다. 각 항목은
...
### 28. 폴더 사용량 리포트 — `#/settings/dir-usage`
...
- **저장**: `dirusage.json`(대상·수신자·SMTP — SMTP 비밀번호는 봉인) · `dirusage.db`(스캔 이력)
- **환경변수**: `DIRUSAGE_TICK_MS`(틱, 기본 60초) · `DIRUSAGE_JOB_TIMEOUT_MS`(엣지 회신 대기, 기본 15분) · `DIRUSAGE_DB_PATH`
- **권한**: `settings`(조회·실행) · **저장과 메일 테스트는 설정 소유자**(SMTP 계정을 다루므로 백업과 같은 등급)
- **주의**:
  ...
  - 메일은 포탈이 **직접 SMTP 로** 보낸다(외부 라이브러리 없음). OAuth2·DKIM·첨부는 지원하지 않는다.

### 29. vCenter 로그 보관 — `#/settings/vclogs`
