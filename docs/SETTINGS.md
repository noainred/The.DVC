...
### 4. iDRAC 서버 등록 — `#/settings/idrac-admin`
...
### 5. 스캔 로그 — `#/settings/idrac-scan-log`
...
### 7. 스토리지 수집 주기 — `#/settings/storage-intervals`
...
### 8. SAN 스위치 포트 사용량 — `#/settings/sansw-perf`
...
### 9. GPU 수집 — `#/settings/gpu-collect`
- **무엇**: ESXi 호스트의 물리 GPU 사용률 수집을 켜고 끈다.
- **저장**: `gpu-physical.json`
- **권한**: `settings`

### 10. GPU 게스트 수집 — `#/settings/gpu-guest`
...
### 14. 원격 법인(DC)에 Edge 노드 포탈 설치 — `#/settings/agent-deploy`
...
### 18. 엣지 사용자 배포 — `#/settings/edge-users`
- **무엇**: 중앙에서 만든 계정을 엣지 포탈들에 일괄 배포한다.
- **저장**: `central-agent-users.json`
- **권한**: `users.manage`

### 19. 인증(AD) — `#/settings/auth-ad`
...
### 20. 세션 보안 — `#/settings/session-security`
...
### 21. 자격증명 저장 방식 — `#/settings/secrets`
...
### 26. NFS 마운트(백업 대상) — `#/settings/nfs-mounts`
- **무엇**: 백업을 저장할 NFS 마운트를 등록·확인한다.
- **저장**: `nfs-mounts.json`
- **권한**: admin

### 27. vCenter 로그 보관 — `#/settings/vclogs`
