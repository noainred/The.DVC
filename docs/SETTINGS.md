...
- **보존**: `IDRAC_RETENTION_DAYS`(기본 90일 — 원본·롤업 공통 상한) · `IDRAC_RAW_RETENTION_DAYS`(v2.451, **원본만** 따로 짧게. 기본 `0` = 기존과 같이 원본도 90일)
...
|---|---|---|---|---|
| **원본 보존일** | `samples`(분 단위) | 90일 | `TEMP_RAW_RETENTION_DAYS` | 실제 측정 시각의 개별 값 |
...
- `0` 을 넣으면 '원본도 보존일을 따름'(v2.450 이전 동작)이 된다.
