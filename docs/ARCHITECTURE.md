...
| `central/` | 38 | 6,440 | `central/inventory.js` (10) | 사이트 위임 수집 — 중앙(OC2) 측 인벤토리 캐시. |
...
| `metrics/` | 6 | 1,163 | `metrics/db.js` (8) | Generic metrics time-series store (host temperature, datastore usage, GPU |
...
| `routes/` | 86 | 21,708 | `routes/capacity.js` (1) | Capacity Advisor API — 포탈/엣지 호스트 리소스 실측·평가·권고 조회. |
...
| `storage/` | 38 | 7,168 | `storage/registry.js` (9) | storage/registry.js — 스토리지 장비 등록부(v2.302). |
...
| `util/` | 65 | 5,523 | `util/numOrNull.js` (76) | `numOrNull` — '읽지 못한 수치' 를 0 으로 둔갑시키지 않는 단일 판정 (v2.561). |
...
디렉터리 63개 · 파일 709개 · 131,643줄. 대표 파일은 `index.js` 가 있으면 그것, 없으면 그 디렉터리 밖에서 가장 많이 import 되는 파일이고, 설명은 그 파일 머리말의 첫 줄을 그대로 옮긴 것이다(따라서 머리말이 곧 문서다 — 첫 줄을 잘 쓸 것).
