...
|---|---:|---:|---|---|
| `agent/` | 40 | 5,188 | `agent/envTimeout.js` (18) | 호환 재수출(v2.613 DEPS2613-01) — 본체는 `util/envTimeout.js` 로 옮겼다. |
...
| `bmusage/` | 18 | 3,489 | `bmusage/poller.js` (3) | bmusage/poller.js — 베어메탈 사용률 주기 수집(v2.550). |
| `capacity/` | 4 | 767 | `capacity/sampler.js` (4) | Capacity Advisor 샘플러 — 운영 중인 포탈 프로세스 안에서 30초마다 자기 호스트를 실측한다 |
| `central/` | 38 | 6,591 | `central/inventory.js` (10) | 사이트 위임 수집 — 중앙(OC2) 측 인벤토리 캐시. |
...
| `gpu/` | 8 | 1,661 | `gpu/guestops.js` (8) | VMware Tools 게스트 작업으로 게스트 OS 안에서 nvidia-smi를 실행해 GPU 사용률을 |
...
| `idrac/` | 29 | 5,997 | `idrac/registry.js` (15) | iDRAC registry — the managed list of Dell servers whose power draw we collect |
...
| `metrics/` | 6 | 1,381 | `metrics/db.js` (8) | Generic metrics time-series store (host temperature, datastore usage, GPU |
...
| `pdu/` | 11 | 1,701 | `pdu/poller.js` (4) | pdu/poller.js — PDU 주기 수집. |
...
| `reports/` | 8 | 782 | `reports/dailyReport.js` (3) | 일일 헬스체크 리포트 발송 스케줄러 — 매일 지정 시각(HH:MM)에 computeHealthReport 결과를 |
| `rma/` | 14 | 2,553 | `rma/jobs.js` (6) | RMA 원격 명령 잡 큐(중앙, 인메모리) — claim→ack 2단계 확인응답(captureJobs.js 패턴 이식) |
| `routes/` | 86 | 21,906 | `routes/capacity.js` (1) | Capacity Advisor API — 포탈/엣지 호스트 리소스 실측·평가·권고 조회. |
...
| `storage/` | 38 | 7,192 | `storage/registry.js` (9) | storage/registry.js — 스토리지 장비 등록부(v2.302). |
...
| `util/` | 67 | 5,694 | `util/numOrNull.js` (78) | `numOrNull` — '읽지 못한 수치' 를 0 으로 둔갑시키지 않는 단일 판정 (v2.561). |
...
디렉터리 63개 · 파일 711개 · 133,045줄. 대표 파일은 `index.js` 가 있으면 그것, 없으면 그 디렉터리 밖에서 가장 많이 import 되는 파일이고, 설명은 그 파일 머리말의 첫 줄을 그대로 옮긴 것이다(따라서 머리말이 곧 문서다 — 첫 줄을 잘 쓸 것).
