...
- **보안** — 스킴 화이트리스트(`javascript:` 저장 차단), **SSRF 가드**(루프백·169.254.169.254 등 차단,
  RFC1918은 허용), 리다이렉트 미추적, `default-src 'self'` CSP, 정적/백업 경로 탈출 차단,
  비밀번호 pbkdf2(21만 회)+로그인 실패 잠금. 포탈 전체를 비공개로 하려면 `HUB_TOKEN` 을 설정합니다.
