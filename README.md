...
- **특수 기능 도구별 접근 (v2.197+, 표시 v2.209.0)** — 45개 특수기능 도구를 역할별로 **개별 차단**(deny-list). '전체허용/전체차단' 일괄 설정 지원. 상단 '특수 기능' 탭은 항상 노출되고, 권한 없는 도구는 숨기지 않고 **회색·클릭 불가(🔒 권한 없음)** 로 표시해 어떤 기능이 있는지 확인하고 관리자에게 요청할 수 있습니다(딥링크로도 열리지 않으며 서버 API 도 별도 강제).
...
CSRF·SSRF 스캐너·정보노출·잠금 DoS 를 차단했습니다. 상세 [설치 가이드 §7](docs/INSTALL.md)·[감사 이력](SECURITY-AUDIT.md).
...
> 인증·데이터가 서로 얽히지 않습니다. 소스는 [`pyportal/`](pyportal/README.md).

```bash
cd pyportal && python3 app.py        # http://<서버>:8095  (pip 설치 불필요)
```

- **언어/의존성** — Python 3.9+ **표준 라이브러리만**. 프런트엔드도 빌드 도구·CDN·차트 라이브러리 없이
  순수 HTML/CSS/JS라 인터넷이 막힌 Rocky 9 서버에 복사만으로 올라갑니다.
- **화면** — ① 서비스 바로가기(즐겨찾기·카테고리·검색·최근 점검 배지) ② 센터 현황(좌표 지도·센터별 전용 링크,
  **표시 개수는 설정에서 지정** — 0이면 전체) ③ **링크 상태 점검 추이**(아래) ④ **⚙ 설정**(비밀번호 보호, 아래).
- **⚙ 설정 = 비밀번호 보호 (v2.212+)** — 상단 우측 [설정] 버튼은 잠겨 있고, 첫 기동 때 임의 생성되어
  `$HUB_DATA_DIR/initial-settings-password.txt`(0600)에 저장된 **초기 비밀번호**로 들어갑니다(변경 시 파일 자동 삭제).
  하위 메뉴 6개: **바로가기 관리 · 데이터센터 구성**(사이트 편집 + **화면 표시 개수 지정**) **· 카테고리 구성**(v2.216) **·
  사용자 구성 및 설정 · 현재 설정 백업 · 알림 & 감사 로그**(v2.215). 목록은 **↑↓ 버튼으로 표시 순서**를 바꿀 수 있습니다.
  로그인은 **사용자명 + 비밀번호**입니다(v2.216 — 이전에는 비밀번호만 받아 비밀번호가 곧 계정 식별자였습니다).
  역할은 `admin`(전체) / `viewer`(바로가기까지). **조회는 공개, 상태 변경은 전부 로그인 필요.**
  세션은 서명 토큰이라 **서비스를 재시작해도 로그인이 유지**됩니다(v2.215).
- **상태 전환 알림 · 감사 로그 (v2.215)** — 링크가 **정상↔장애로 바뀌는 순간에만** 웹훅으로 알립니다
  (연속 실패 임계·최소 간격으로 플래핑 억제, 기동 직후 첫 관측은 알리지 않음). 로그인·설정 변경·백업 인출은
  `audit.log`(JSON Lines, 0600, 회전)에 남고 설정 화면에서 조회합니다(admin 전용, 비밀값은 기록하지 않음).
- **정기 백업** — 설정 파일 스냅샷을 **주기(30분~1주)** 마다 자동 생성하고 **보관 수량**을 넘으면 오래된 것부터 삭제.
  즉시 백업·받기·복원 지원(복원 직전 상태도 자동 백업).
- **카테고리 직접 구성 (v2.216)** — 바로가기 분류를 설정에서 만들고 이름·색상(8종)·순서를 정합니다.
  대시보드 필터 칩에 즉시 반영되며, 분류를 지워도 그 바로가기는 기본 카테고리로 옮겨져 사라지지 않습니다.
- **바로가기 CSV 가져오기/내보내기 (v2.216)** — 엑셀에서 정리한 표를 그대로 올리고(같은 URL은 건너뜀),
  내보낸 CSV는 백업으로도 씁니다. RFC 4180 파서 + CSV 인젝션 방어 + UTF-8 BOM.
- **점검 이력 DB + 기간별 차트** — 자동 점검 결과(기본 **포트 응답 여부**, 설정에서 HTTP 코드 기준으로 변경 가능)를 **SQLite**에 적재하고
  **5분/10분/30분/1시간/6시간/24시간/일주일/한달** 버튼을 고르면 즉시 해당 구간의 **가용률·평균 지연 추이**를
  SVG 차트로 보여줍니다(개별 링크 필터 지원).
- **저장** — 설정 JSON 4종(0600, 원자적 쓰기, 손상 시 `.corrupt.<ts>` 보존) + 이력 SQLite(WAL, `ts` 인덱스).
- **보안 (6차 감사 v2.214 반영)** — 스킴 화이트리스트(`javascript:` 저장 차단), **SSRF 가드**(루프백·
  169.254.169.254 등 차단, RFC1918은 허용), 리다이렉트 미추적, `default-src 'self'` CSP, 정적/백업 경로
  탈출 차단, 비밀번호 pbkdf2(21만 회). 여기에 **CSRF 차단**(쿠키는 조회 전용 + 교차출처 상태변경 403),
  **임의 URL 점검 로그인 필수**(사내망 스캐너 악용 차단), **미인증 응답에 서버 경로 미노출**,
  **IP별 로그인 잠금**(전역 잠금으로 관리자가 밀려나지 않게), **소켓 타임아웃·동시연결 상한**을 적용했습니다.
  포탈 전체를 비공개로 하려면 `HUB_TOKEN` 을 설정합니다.
- **두 포탈 상호 링크 (v2.215)** — 서로 주소를 모르는 별도 프로세스라 env 로 알려 줍니다.
  포탈에 `SERVICE_HUB_URL` 을 주면 **특수 기능**에 '서비스 허브' 카드가, 허브에 `HUB_PORTAL_URL` 을 주면
  헤더에 **[모니터링 포탈 ↗]** 이 나타납니다. 인증은 각자 유지합니다(연동하지 않는 편이 안전).
- **상시 구동** — `pyportal/systemd/dc-service-hub.service` 참고(전용 계정 `dchub`, `ProtectSystem=strict`).
- 오프라인 설치 패키지/업그레이드 번들에도 `pyportal/` 이 함께 포함됩니다(기동은 운영자가 선택).
