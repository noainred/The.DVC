...
### 모니터링 / 대시보드
- **글로벌 개요** — 전세계 KPI(vCenter/호스트/VM/CPU/메모리/스토리지/전력/GPU)와 **서버·게스트 수량**(서버 합계 = 물리 전용 + 가상화 호스트 — 같은 장비를 두 번 세지 않음, 법인별 표, v2.526~2.527), 법인별 VM·용량·리전별 워크로드 차트. 세계 지도는 v2.526 에 Overview 에서 제거됐고 관제 콘솔 화면에만 남아 있다. 법인별 표의 물리 전용 서버는 v2.583 부터 관리자 지정(서버 분석 › 구분)과 법인(DataCenter)의 유일한 vCenter 로도 귀속되고, 정할 수 없는 서버는 '법인 · vCenter 미지정' / '법인 미귀속' 행으로 표 아래에 따로 보인다.
...
- **기능별 권한 매트릭스 (v2.196+)** — 역할은 3개로 두되 **기능 단위 권한 키 18종**(대시보드·인벤토리 6종·특수기능·인사이트·서비스 모니터(svcmon, v2.506)·원격접속·원격콘솔·VM 사양변경/프로비저닝·게스트계정 배포·설정·업그레이드·사용자관리 — `server/src/auth/permissions.js` 가 진실의 원천)을 **설정 › 사용자 관리에서 체크박스로 켜고 끕니다**. 서버(`requirePerm`)와 WS SSH/RDP 게이트웨이가 실제로 강제하므로 메뉴를 숨겨도 API 직접 호출은 차단됩니다. admin은 항상 전체(잠김 방지). 기본값은 기존 role 동작과 동일해 도입만으로 권한이 바뀌지 않습니다.
- **특수 기능 도구별 접근 (v2.197+, 표시 v2.209.0)** — 특수기능 도구(v2.583 기준 86종, `web/src/views/specialToolsList.js` 단일 소스 — v2.573 부터 역할 표가 관리자 표시 도구까지 카탈로그 전체를 보인다)를 역할별로 **개별 차단**(deny-list). 사용자 단위로는 **허용 목록 모드**(고른 도구만 보이고 나머지는 숨김 — 앞으로 추가되는 도구도 차단)와 거부 목록 모드를 고를 수 있다(v2.555). '전체허용/전체차단' 일괄 설정 지원. 상단 '특수 기능' 탭은 항상 노출되고, 권한 없는 도구는 숨기지 않고 **회색·클릭 불가(🔒 권한 없음)** 로 표시해 어떤 기능이 있는지 확인하고 관리자에게 요청할 수 있습니다(딥링크로도 열리지 않으며 서버 API 도 별도 강제).
...
주요 항목만 발췌. 전체 목록·기본값·정의 위치는 소스에서 생성하는 [docs/ENV.md](docs/ENV.md)를 볼 것 — `server/src/config.js` 는 핵심 키만 읽고 도메인 전용 키는 각 모듈이 직접 읽는다.
...
| `CSP` | (기본 정책 적용) | Content-Security-Policy. v2.577 부터 **기본 켜짐** — `off` 로 끄고, 정책 문자열로 바꾼다(`/intro` 데모 페이지만 별도 완화 정책). 세션 토큰이 localStorage 에 있으므로 끄는 것은 권장하지 않는다 |
...
| `esxitemp` | 서버 온도(iDRAC/ESXi — 물리·가상화 구분·5년 추이. 옛 이름 'ESXi 온도') | `hardware` / `hba` | 벤더·모델 / HBA 속도 |
...
### 최근 추가·변경 (v2.540 ~ v2.583)

특수 기능 카드는 v2.583 기준 **86개**다(`web/src/views/specialToolsList.js` 가 진실의 원천 — 관리자 표시 25개 포함.
설정 › 특수 기능 카테고리로 섹션을 묶을 수 있다). 버전별 상세는 `server/src/release-notes.json`(포탈 안 릴리스 노트 화면).

- **v2.583 — 로그 분석(개선점)**(설정 › Log › 로그 분석, `#/settings/log-analysis`): 로그를 반출할 수 없는 폐쇄망에서
  포탈 로그를 읽어 개선점을 뽑는다. 원천 5가지 — **누적**(이 포탈 로그를 시간 버킷으로 최대 7일 누적, `log-analysis-stats.json`
  에 보존, `LOGANALYSIS_LIVE=0` 이면 끔) · **최근 로그**(메모리 1,000줄) · **서비스 저널**(`journalctl -u`, 유닛은
  `PORTAL_SYSTEMD_UNIT` 기본 `vmware-portal` — 서비스 계정이 `systemd-journal` 그룹이 아니면 읽지 못하고 화면이 그 조치를 말한다) ·
  **붙여넣기**(8MB·20만 줄까지) · **엣지 로그**(특수기능 › 엣지 로그에서 가져온 보관분). 결과는 규칙 카탈로그(의미·조치·바로가기) ·
  HTTP 5xx/413/429/401·403/느린 응답(경로별) · 규칙에 없는 경고(수준이 없는 원천은 키워드로 추정 — 그 사실을 밝힌다) · 로그 잡음.
  관리자 + 전체 범위 계정 전용(로그에 전 법인 호스트명·IP 가 들어간다).
- **v2.583 — '불러오는 중…' 에 지연 주체 표시**: 오래 기다리는 요청마다 메서드 + 식별자를 가린 경로 + **요청 ID** + 서버 쪽 상태
  (서버 처리 중 / 서버는 이미 응답 / 서버에 기록 없음)를 보인다. 브라우저가 `X-Request-Id` 를 보내고 서버가 같은 값을 응답 헤더 ·
  서버 성능 측정의 느린 요청·hang 기록 · 라이브 로그 줄 끝(`#<ID>`)에 남긴다. 상태 조회 `GET /api/perf/req-status?ids=`(본인 요청만,
  관리자는 전부), 설정 › Log › 서버 성능 측정에 '요청 ID 찾기'.
- **v2.583 — Overview 법인별 물리 전용 서버**: 법인별 표의 물리 전용 열이 전부 0 이던 결함 수정(위 '글로벌 개요' 참조).
  GPU 게스트 수집 자동 모드는 실패 시 두 경로의 사유를 모두 남긴다(`게스트작업: … / SSH: …`).
- **통신·포탈 점검**: 통신 점검(`link-check`, v2.552 — 중앙↔엣지·vCenter 링크를 DNS→TCP→TLS→HTTP→인증→정체 단계로 재고
  3단 로그로 기록, 기본 꺼짐) + 설정 전수 점검(v2.553 — 등록된 25종 접속처를 **장비 계정으로 로그인하지 않고** 재고 해결책 제시) ·
  포탈 점검(`portal-check`) › 토큰 점검(v2.560) · 인벤토리 점검(v2.570 — 위임 vCenter 수신 낡음·거부) ·
  엣지 로그·진행상태(`edge-log`, v2.549 — 누를 때만 엣지에서 당겨 온다).
- **베어메탈 사용률**(`bm-usage`, v2.550~2.554): 서버 분석 › 구분에서 Baremetal 로 분류된 서버만 CPU·메모리·디스크·네트워크·HBA 를
  iDRAC 텔레메트리 + OS SSH(Linux/Windows)로 수집. 법인 단위 opt-in(기본 꺼짐) · 원시 90일 + 일 롤업 5년 · 임계 알림(기본 꺼짐) ·
  Enterprise 라이선스 서버는 관리자 동의 후 대체 경로(v2.554).
- **파트 장애(부품)**(`part-faults`, v2.547 → v2.548 재설계): 서버·스토리지·SAN 부품 장애를 엣지에서 판정해 상태 전이만 기록하고
  파트당 즉시 알림. '확인 불가' 를 정상으로도 장애로도 세지 않는다. 기본 꺼짐.
- **권한·화면**: 사용자별 특수기능 허용 목록 모드(v2.555) · 역할별 도구 표가 카탈로그 전체를 보임(v2.573) · 서버 온도 히트맵 보드와
  상단 메뉴 2단화(v2.556, 1600px 이상은 한 줄 — v2.572) · 수집 서버 '빈 인벤토리' 원인 모달(v2.560)·진단 모달 재구성(v2.571).
- **외부 포탈 조회 API**(`/api/v1`, v2.562): 조회 전용 · 허용 목록 · 전용 API 키(설정 › Security › 연동 키). 연동 문서는
  [docs/API-PUBLIC.md](docs/API-PUBLIC.md). v2.564 의 무인증 안내 페이지는 v2.565 에 철회됐다.
- **Unity 스토리지**(v2.540~2.546): 용량 산정(더 줄 수 있는 용량을 세 기준으로) · uemcli 파싱 재작성(명령 26개 → 3개) · SSH 는 PTY 없이
  실행(sudo 프롬프트 정지 원인) · 모델·버전 수집 · 다중 풀.
- **감사·점검 릴리스**(v2.574~2.582): 확정 결함 수정 · CSP 기본 적용(v2.577) · 오픈소스 고지 `THIRD-PARTY-NOTICES.txt`·루트 `LICENSE`(v2.576) ·
  기간별 값 차이 설명(v2.578) · 아키텍처 점검 1~4차(v2.579~2.582). 기록은 [docs/INDEX.md](docs/INDEX.md) §5 의 감사 문서.
- 이 절 이전에 추가됐지만 위 표에 없는 도구: 전산실 운영 온도(`roomtemp`, v2.381) · SAN 스위치 모니터링(`san-switch`, v2.410) ·
  폴더 사용량 Top-N(`dir-usage`, v2.454) · 메일 진단(`mail-diag`, v2.455) · 고아 VMDK 찾기(`orphanvmdk`, v2.505) ·
  현재 사용자(`curuser`, v2.520) · 스토리지 증가량(`storage-growth`, v2.531).

### 최근 추가·변경 (v2.445 ~ v2.478)

(v2.478 시점 기록 — 현재 카드 수는 위 v2.540~v2.583 절 참조) 위 목록에 없는 도구:
...
- **신규 포탈(V4)**(`web/src/version_4/`, v2.508 — v2.490 의 V3 를 승격): UI 개편 시안(`Design/uiredesign/` 아트보드 8장)을 구현한 라이트 테마 셸. 설정 › 신규 포탈 보기 › **V4** 버튼 또는 `#/v4` 로 진입하고, 기존 개발 포탈은 그대로 유지한다. 옛 `#/v3` · `#/v3/<화면>` 은 `#/v4/...` 로 자동 이동한다.
  ...
  - **좌측 내비 = IA 트리 11그룹**(`version_4/tree.js`): 특수 기능 전부(v2.583 기준 86개) + 개발 포탈 탭 11개 + 화면 9개를 **한 트리에 1회씩** 배치(미분류 0, 중복 소속은 '별칭' 표기). 도구 화면은 개발 포탈 `#/tools/<키>` 에서 열리며 **키 문자열은 바꾸지 않는다**(toolsDenied 값이자 서버 집행 매핑).
...
**설정** 탭은 7개 그룹 + 단독 메뉴로 구성된다(v2.583 기준 화면 43개 — `web/src/views/Settings.jsx` 의 `SUB` 가 진실의 원천,
화면별 상세는 [docs/SETTINGS.md](docs/SETTINGS.md)). 하위 화면은 `#/settings/<키>` 로 바로 열린다.

| 그룹 | 하위 메뉴 |
|---|---|
| 🖥️ **vCenter 관리** | vCenter 등록·관리 · vCenter 연결 테스트 |
| 🗄 **수집 서버** | iDRAC 서버 등록 · **스캔 로그**(주기/수동 스캔 실행 이력, 통합/법인별) · 지표 수집 · 스토리지 수집 주기 · 전원 꺼짐 점검 · VM 실시간 스파이크 수집 · SAN 스위치 포트 사용량 · 게스트 계정 추가 · 수집 서버(원격) · **원격 법인(DC)에 Edge 노드 포탈 설치** |
| 🎮 **GPU 사용량 수집** | GPU 수집 · GPU 게스트 수집 · GPU 수집 진단 |
| 🔌 **원격 접속 서버** | 중계 서버 · 원격접속 설정 |
| 👤 **User Control** | 메인포탈 사용자 관리(계정 CRUD·OTP 등록/해제·**비번 설정/로그인 차단**·**데이터 범위(scope)**·**기능 권한 매트릭스**·**특수기능 도구별 접근**) · **엣지 사용자 배포** · 인증(AD) |
| 🛡️ **Security** | 세션 보안 · 자격증명 저장 방식 · 연동 키(외부 포탈 조회 API) · 이상동작 탐지 · 호스트 접근 제어 · 보안 자가진단 |
| 📋 **Log** | vCenter 로그 보관 · 진단·로그 · 서버 성능 측정(요청 ID 찾기 포함) · **로그 분석(개선점)**(v2.583) · 감사 로그 |

그 외 단독 메뉴: DataCenter(법인) · NSX 관리 · AI 검색 · 알림 · 메일 발송 · 특수 기능 카테고리 · 신규 포탈 보기 · 포탈 백업 · NFS 마운트(백업 대상) · 폴더 사용량 리포트 · ⬆ 업그레이드 · About.
...
- **에이전트 ↔ 수집 서버 자동 연결(v2.434, 설정 › 에이전트 배포 › 에이전트 현황)**: 엣지에 깔리는 프로그램은
  하나(`vmware-portal`)지만 중앙은 그 하나를 **설치용 SSH 대상**과 **pull 용 URL+토큰** 두 목록에 적어 둔다.
  후자가 배포 성공 시에만 생성돼 '대상만 저장' 한 사이트는 수집이 안 되던 문제를 두 가지로 메운다 —
  ① 대상을 **저장만 해도** 수집 서버로 자동 등록(토큰 없으면 생성) ② **🔗 연결 상태** 카드가 두 목록을 대조해
  `연결됨/수집 서버 없음/토큰 없음/URL 불일치/토큰 불일치` 를 표로 보이고 빠진 것을 **일괄 추가**한다
  (`agent/collectorSync.js`, 매칭은 접속 URL 우선 → 수집 서버 id). 추가 시 토큰 생성·**엣지에 SSH 반영**·
  연결 확인을 고를 수 있고, 최대 50건·동시 3건. 배포 대상이 없는 수집 서버(수동·자기등록)도 함께 보여준다.
- **Edge 노드 대량 배포(v2.432, 설정 › 에이전트 배포 › 대량 배포)**: 엑셀·위키의 서버 목록을 붙여넣으면
  ...
  **대상 등록 없이 바로 설치**한다(`agent/deployText.js`·`agent/bulkDeploy.js`). 계정·키·중앙 URL·토큰 등
  ...
  저장된 대상을 같은 텍스트/**CSV** 형식으로 내보내 왕복 편집할 수 있다(비밀 제외, `?secrets=1` 은 설정 소유자).
  **열 순서**는 화면에서 고른다(v2.433) — `host·이름·계정·비밀번호`(기본)·**`host·계정·비밀번호`(ip id pw)**·
  **`host·비밀번호`(ip pw)** 등. 3열 표는 값만으로 '이름/계정'과 '계정/비밀번호'를 구분할 수 없어 추측하지 않는다.
  `root@10.1.1.1:2222` 처럼 계정을 host 에 붙여 써도 되고, 헤더 행이 있으면 헤더가 우선한다.
  **토큰 자동**(v2.433): 중앙 토큰은 이 포탈 값을 자동으로 넣고(없으면 화면에서 생성), 수집 토큰은
  **노드마다 다른 난수**를 만든다(한 값 공유 시 한 대 유출로 전 사이트가 열리므로).
...
- 자격증명/시크릿(`vcenters.json`, `users.json`, `*-assignments.json`, `central-agent-*.json`, `horizon.json`, 스캔/게스트 계정 등)은 `CONFIG_DIR`에 `0600` + **원자적 쓰기**(tmp+fsync+rename)로 저장되고 API 응답에서 마스킹됩니다. 손상 시 `<파일>.corrupt.<ts>`로 보존해 조용한 유실을 막습니다. 운영 시 `AUTH_SECRET` 지정 + 최초 임의 비밀번호(`initial-admin-password.txt`) 변경 필수. 보안 응답 헤더(X-Frame-Options·nosniff·Referrer-Policy · **CSP 기본 켜짐**(v2.577) · HSTS 는 실제 HTTPS 일 때만)·CORS 기본 차단·업그레이드 sha256 필수 등은 [설치 가이드 §7](docs/INSTALL.md) 참고.
- **역할(RBAC) + 기능 권한** — `admin` / `operator` / `viewer` 3역할에 **기능 권한 매트릭스**(18개 키)를 얹어 설정 › 사용자 관리에서 역할별로 조정합니다. 상태변경 API와 **브라우저 SSH/RDP 터널**은 해당 권한(`remote.access` 등)이 있어야 하며, 기본값은 종전대로 `admin`·`operator`만 가능하고 viewer는 조회 전용입니다. **서버가 진실의 원천**이라 UI에서 버튼을 숨기는 것과 별개로 API 직접 호출도 차단됩니다. 비밀번호·역할 변경·계정 삭제·로그인 차단 시 그 계정의 **기존 토큰이 즉시 무효화**됩니다.
