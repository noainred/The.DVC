...
- **언어/의존성** — Python 3.9+ **표준 라이브러리만**. 프런트엔드도 빌드 도구·CDN 없이 순수 HTML/CSS/JS라
  인터넷이 막힌 Rocky 9 서버에 복사만으로 올라갑니다.
- **화면 4개** — ① 서비스 바로가기(즐겨찾기·카테고리·검색) ② 28개 센터 현황(좌표 지도·센터별 전용 링크)
  ③ 링크 상태 점검(서버가 각 URL에 실제 요청 → 응답코드·지연, 동시 실행) ④ 설정(생성/수정/삭제·JSON 내보내기·가져오기).
- **저장** — `shortcuts.json` 한 개(0600, 원자적 쓰기, 손상 시 `.corrupt.<ts>` 보존).
- **보안** — 스킴 화이트리스트(`javascript:` 저장 차단), **SSRF 가드**(루프백·169.254.169.254 등 차단,
  RFC1918은 허용), 리다이렉트 미추적, `default-src 'self'` CSP, 정적 경로 탈출 차단. 접근을 좁히려면
  `HUB_TOKEN` 환경변수를 설정합니다.
