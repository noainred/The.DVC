...
## 2026-09-13 보안 감사 조치 — 되돌리지 말 것

- **로그인 실패는 자격 종류·계정 존재와 무관하게 같은 비용을 쓴다**(`auth.py` `_burn_password_work`):
  없는 계정·중지 계정은 PBKDF2 를 돌리지 않아 응답이 ~100배 빨랐다(실측 128ms vs 1.5ms). 문구를 같게
  맞춘 것만으로는 **타이밍으로 계정 열거**가 되므로, 그 경로에서도 더미 해시를 1회 태운다.
- **`hmac.compare_digest` 에 str 을 넣지 말 것**(`server.py _hub_token_ok`): 비-ASCII 인자면 TypeError 다
  (파이썬 사양). 헤더는 latin-1 로 디코딩되므로 `X-Hub-Token: évil` 한 줄로 발생했고, 이 호출이 `_dispatch`
  의 try 밖이라 **응답 없이 연결이 끊겼다**. 바이트로 비교하고 예외는 False 로 접는다.
- **백업 목록도 admin 전용**(`server.py api_backup_list`): 목록만 일반 세션에 열려 있어 백업 파일명·개수와
  저장 디렉터리 절대경로가 노출됐다 — 이 문서가 규정한 "목록·다운로드·복원은 admin 세션만" 과 어긋났다.
  형제 라우트(생성·다운로드·복원·삭제)와 같은 경계를 유지할 것.
- **웹훅 URL 은 값이 곧 자격증명**(`server.py api_settings_state`): Slack/Teams 웹훅 URL 을 비-admin 에게
  내려주지 않는다(저장 여부만 `hasWebhookUrl`). 전송은 admin 전용인데 값 읽기가 열려 있으면 경계가 어긋난다.
- **sqlite 파일도 0600**(`history.py`): 다른 데이터 파일은 전부 0600 인데 sqlite 생성 파일만 umask 기본
  (0644)이었다. 점검 이력에는 등록 링크·내부 호스트가 담긴다.
