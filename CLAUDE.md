...
- **작업 시작 시 난이도 + 적합 모델 + 현재 실행 모델 표시**: 모든 작업(명령)을 시작할 때
  응답 맨 앞에 난이도·권장 모델·현재 실행 중인 모델을 한 줄로 표시한다.
  형식: `난이도: 낮음/보통/높음 — 적합 모델: Sonnet/Opus | 현재 실행: <현재 모델>`.
  (단순 편집·UI·엔드포인트=낮음/보통→Sonnet, 아키텍처·동시성·대규모 리팩터·미해결 버그=높음→Opus)
  현재 모델과 적합 모델이 다르면(예: 낮은 난이도를 Opus로, 높은 난이도를 Sonnet으로) 한 줄로 알려준다.
