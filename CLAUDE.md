...
- **테스트에서 `Date.now()` 를 기준 시각으로 쓰지 말 것 — 시각에 따라 깨진다**(v2.517 에 CI 실패로
  ...
  ⚠ 남은 플래키 2건(`perfMonitor2498.test.js:388`, `chunkedPrune2453.test.js:48`)은 원인 미확정이다.
...
- 적용된 성능 메커니즘(회귀 방지 — 유지할 것):
  ...
  - **월간 점검은 '확인 불가' 를 '이상 없음' 으로 뭉개지 않는다**(`sanswitch/healthCheck.js` +
    ...
    - **새 명령 4종의 파서는 실장비 출력을 확인하지 못했다**(`sensorshow`·`errdump`·
      ...
      각 파서 머리말의 고지를 지울 것.
    - ⚠ **`errshow` 를 수집에 쓰지 말 것** — FOS 의 `errshow` 는 **대화형**(페이저로 입력을 기다린다)
      이라 폴러가 부르면 캡처가 시한까지 매달린다. 비대화형 `errdump` 가 같은 내용을 준다.
