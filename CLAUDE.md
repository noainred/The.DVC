...
- 적용된 성능 메커니즘(회귀 방지 — 유지할 것):
  ...
  - ⚠⚠ **로그인 없이 보는 공개 API 안내 페이지 — 나가는 것은 문서뿐이고, 내부 API 는 싣지 않는다**
    (`publicapi/{samples,docsSettings}.js` · `routes/publicDocs.js` + 웹 `views/ApiDocs.jsx`·
    `apiDocsText.js`, v2.564 — 사용자 요청 "로그인 없이 볼 수 있는 페이지를 하나 만들어서 API 를
    찾아서 사용할 수 있는 페이지 만들어줘, 사용 예시와 샘플을 같이 제공하는 기능"):
    - ⚠⚠ **`routes/publicDocs.js` 는 스냅샷·DB·등록부를 import 하지 않는다.** 여기서
      `store.get()` 한 줄이면 **무인증으로 vCenter 이름·대수가 샌다**. 카탈로그는 `allowlist.js`
      (선언), 샘플은 `samples.js`(손으로 지어낸 값)에서만 온다 — 테스트가 **import 목록을 화이트
      리스트로** 고정하고 목 생성기의 id·이름이 응답에 하나도 없음을 확인한다.
    - ⚠⚠ **내부 API 812개를 싣지 않는다.** 공개하는 것은 `/api/v1` 조회 전용 8개뿐이다. 전량을
      무인증으로 내보내면 기존 불변조건 셋과 정면으로 어긋난다 — ① `/api/v1` 404 는 경로 목록을
      주지 않는다(열거 단서) ② 미인증 응답에 계정명을 싣지 않는다 ③ `x-powered-by` 제거.
      RMA 원격 명령·프로비저닝·방화벽 변경·백업 다운로드 경로의 **이름이 드러나는 것 자체가**
      공격자에게 지도를 주는 일이다.
    - ⚠ **끄면 404 다 — 403 이 아니다**(`docsSettings.js`). 403 은 "여기 뭔가 있는데 막혔다" 를
      알려주므로 감추는 목적에 어긋난다. env `PUBLIC_API_DOCS` 는 **끄는 쪽으로만** 우선한다 —
      `'true'` 로 되살리는 분기를 만들면 관리자가 끈 결정이 조용히 뒤집힌다(테스트가 소스를 검사).
    - ⚠ **샘플은 합성값이고 그 사실을 화면이 말한다.** 식별자는 전부 `demo` 표식을 단다(테스트가
      `id`·`vcenterId`·`entity` 를 검사) — 표식 없는 그럴듯한 이름을 넣으면 연동 담당자가 '이
      포탈의 실제 장비' 로 읽는다. ⚠ 다만 `/faults/alarms` 의 `name` 은 **vSphere 알람 정의 이름**
      이라 검사 대상이 아니다(실제 응답에도 그 문자열이 그대로 온다).
      샘플에 **일부러 `null` 과 `pending` vCenter 를 넣어 둔다** — 전부 값으로 차 있으면 연동하는
      쪽이 null 을 만날 준비를 하지 않는다(이 API 의 계약이 "읽지 못한 값은 null" 이다).
    - ⚠ **샘플의 필드 집합 == `allowlist.js` 의 선언 `fields`** 를 테스트가 대조한다. 어긋나면
      문서가 **없는 필드를 약속**하거나 있는 필드를 감춘다 — v2.553 '조언이 틀리면 무음 실패보다
      나쁘다' 와 같은 계열이다.
    - ⚠ **오류 코드표는 서버가 실제로 내는 코드와 1:1** 이다(테스트가 `auth.js`·`allowlist.js`·
      `publicApi.js` 에서 뽑아 대조). 한쪽만 늘면 화면이 모르는 코드를 그대로 보여주거나 **없는
      코드를 안내한다**. v2.564 에 이 검사가 `internal`(500) 누락을 바로 잡아냈다.
    - **'직접 호출' 의 키는 `useState` 에만 둔다** — `localStorage` 에 넣지 말 것. 이 페이지는
      **로그인 없이** 열리므로 공용 PC 에서 다음 사람이 그 키로 전 함대 조회 데이터를 읽는다.
      예시 코드도 자리표시자·환경변수만 쓴다(테스트가 키처럼 보이는 40자 이상 조각을 금지).
    - ⚠⚠ **`BoldText` 를 쓰지 말 것.** 그것은 `**강조**` 만 해석해 **백틱이 글자로 샌다** — 이
      화면의 원문(`CONTRACT_NOTES`)은 `null`·`meta.truncated` 를 백틱으로 적는다. v2.564 초판이
      BoldText 를 써서 **백틱 18개가 그대로 보였다**(Chromium 판독이 잡았다. 수치로는 안 잡힌다).
      전용 `parseRich` 는 **중첩을 처리한다** — `**0 이 아니라 \`null\`**` 처럼 굵게 **안에** 코드가
      든다(6줄 중 3줄). 굵게만 떼고 끝내면 그 안의 백틱이 남는다. 짝이 안 맞는 표시는 **글자
      그대로** 둔다(억지 해석은 원문과 다른 것을 보여준다).
    - ⚠⚠ **`Number(null) === 0` 함정을 일곱 번째로 밟았다**(v2.564 자체 테스트가 잡았다):
      `statusTone(null)` 이 `Number.isFinite(Number(null))` 을 통과해 **초록('성공')** 을 돌려줬다 —
      상태가 없는데 화면이 성공 색으로 칠하는 **오류 없이 틀린 값**이다. **타입부터 좁힌다**
      (숫자이거나 숫자 문자열일 때만 — v2.556 `tempNum` 과 같은 형태).
    - ⚠⚠ **로그아웃 상태에서는 `hashchange` 리스너가 등록되지 않는다**(v2.564 Chromium 이 잡은
      확정 결함): `App.jsx` 의 기존 리스너는 **조기 return 아래**에 있어, 로그인 화면의
      '⇗ API 안내' 를 누르면 주소만 `#/api-docs` 로 바뀌고 **화면은 그대로**였다(직접 주소로
      들어가면 열려서 첫 검증에서 못 봤다). 이 화면의 존재 이유가 '로그인 없이 찾아 들어오는 것'
      이라 그 경로가 죽으면 기능이 통째로 무의미하다. 전용 `apiDocsOn` 상태를 **조기 return 위**에
      선언한다(훅 개수가 달라지면 React #310 — v2.202 사고).
    - 화면 분기는 `if (!user) return <Login…>` **앞**에 둔다(뒤에 두면 로그인 화면이 가로챈다) 그리고
      `user === 'loading'` 앞이기도 하다 — 부팅 조회(`auth/config`·`auth/me`)가 실패하는 환경에서도
      열려야 한다(이 화면은 그 조회가 필요 없다. 부르는 것은 무인증 `/api/docs` 하나뿐).
      `lazy()` 를 유지할 것 — 로그인한 사용자는 평생 열지 않는 화면이다.
    - ⚠ **상태 파일(`public-api-docs.json`)을 `.gitignore` 에 함께 등록**했다. 비밀은 없지만
      '이 현장이 껐다' 는 운영 결정과 조작자 계정명이 담기고 이 저장소는 공개다(v2.535 규약).
    - ⚠ **정직 기록**: 목 서버(인증 켬)에서 `/api/docs` 200 · `/api/health`·`/api/summary`·`/api/vms`·
      `/api/admin/api-keys`·`/api/tools/storage`·`/api/v1/inventory/summary` **전부 401** 을 실제 HTTP 로
      확인했고 목 vCenter 이름 검색 0건이었다. Chromium 1440/400 에서 가로 넘침 0 · pageerror 0 ·
      별표 0 · 백틱 0 · 로그인 화면에서 눌러 실제로 열리는 것까지 확인했다. **실제 외부 연동
      담당자가 이 페이지로 연동에 성공하는 것은 확인하지 못했다.**
