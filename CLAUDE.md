...
- 적용된 성능 메커니즘(회귀 방지 — 유지할 것):
  ...
  - ⚠⚠ **스토리지 '장애 장비' 카드 — '확인 불가' 를 정상으로도 장애로도 세지 않는다**
    (`web/src/views/tools/storageFaultText.js` + `StorageMonTool.jsx`, v2.567 — 사용자 요청
    "스토리지에 장애 있으면 장애 장비 숫자를 카드로 하나 보여주고, **미해결정보를 빼고**, 장애
    장비 수량을 보여주고 클릭하면 장애 장비만 리스트로 보여줘". 스크린샷: Isilon 노드 열의 `24 ⚠1`):
    - **빼낸 것은 '미해결 경보' KPI 다.** 그 칸은 전 장비 경보 합(이 현장 5,404)이라 **숫자가 커서
      눈에 띄는데 조치로 이어지지 않았다** — 장비 몇 대가 문제인지 알 수 없었다. 되살리지 말 것.
    - ⚠⚠ **판정은 세 가지다** — `fault` / `ok` / **`unknown`(판정 근거가 없다)**. `unknown` 을
      정상에도 장애에도 넣지 않는다(v2.519·v2.523·v2.526·v2.534·v2.546 와 같은 규약). 근거가
      하나도 없으면 `ok` 가 아니라 `unknown` 이다 — **'신호 0개 = 정상' 으로 접으면 수집 방식이
      노드를 안 주는 장비(`미수집…`)가 전부 초록이 된다.** 항등식 `합계 = 장애 + 정상 + 확인 불가`
      를 테스트가 고정한다.
    - ⚠⚠ **수집 실패(`snap.ok === false`)는 장비 장애가 아니다** — `unknown` 이다(장비가 고장났는지
      알 수 없다). 그것은 옆 칸 '수집 실패/대기' 가 센다. 두 칸이 같은 장비를 두 번 세지 않는다.
    - ⚠ **장애 0 인데 확인 불가가 있으면 초록으로 칠하지 않는다**(`faultKpiTone` → `muted`) —
      숫자는 '문제 없음' 이라 하고 색도 '문제 없음' 이라 하면 **확인 못 한 장비가 사라진다**.
      부제(`faultKpiMeta`)가 `확인 불가 N대 (정상이라는 뜻 아님)` 을 **항상** 적는다.
    - ⚠ **필터가 0대일 때 '왜' 를 말한다**(`faultFilterNote`) — 법인·종류 칩과 겹쳐 0 이 될 수
      있으므로 '장애가 없다' 와 '이 필터 범위에 없다' 를 구분한다(조용한 빈 목록 금지).
    - ⚠⚠ **표의 `status` 열은 '수집 성패' 이고 장비 장애가 아니다 — 라벨이 `수집 상태` 여야 한다**
      (v2.567 스크린샷 판독에서 잡은 실제 결함. **수치로는 안 잡혔다**): v2.566 까지 라벨이 그냥
      `상태` 라서 노드 1대가 고장난 장비의 행이 **초록 `정상`** 으로 보였다. '장애 장비만' 필터를
      켜면 그 행만 남으므로 화면이 **"장애 1대" 라고 말한 바로 옆에서 "정상"** 이라고 말한다 —
      v2.526 `healthBadge`·v2.542 `sectionBadge` 와 같은 계열이고 **사람은 색을 먼저 읽는다**.
      배지의 `title`("수집 성공 — 장비 부품 상태가 아닙니다")도 함께 유지할 것.
    - **판정은 순수 모듈 하나가 소유한다** — KPI·필터·법인 칩 툴팁이 같은 `deviceFault()` 를 쓴다.
      20줄을 복사하면 카드의 개수와 필터 결과가 갈라진다(CLAUDE.md '코어는 하나다').
    - `Kpi` 에 `title` 을 더했다(기본값은 예전 문구라 기존 181개 KPI 회귀 0).
