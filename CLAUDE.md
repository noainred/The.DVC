...
- 적용된 성능 메커니즘(회귀 방지 — 유지할 것):
  ...
  - **Unity 수집의 진짜 원인은 '인증서 확인 프롬프트' 였다 — 파싱 결함은 별건이었다**(`proxy/sshExec.js`
    ...
    - ⚠⚠ **`uemcli` 후보의 첫 자리는 반드시 `-output csv` 다**(v2.529 — v2.526 회귀 수정.
      사용자 화면 실측 OC2-41.237): v2.526 이 사용자 캡처(`pool -detail` **평문**)를 보고 평문을
      첫 후보로 바꿨더니 현장에서 **풀·시스템이 파싱되지 않았다** — `capacity: 오류: 풀 출력에서
      용량 필드를 인식하지 못했습니다` · `config: 건너뜀`. **같은 세션의 `alerts` 는 OK** 였으므로
      SSH·명령 실행은 멀쩡했고 **파싱만** 실패한 것이다. v2.525 는 CSV 가 첫 후보였고 잘 돌았다.
      `recordsFor` 는 CSV 를 훨씬 안정적으로 읽는다(평문 `-detail` 은 `N:` 접두 + `Key = Value` 라
      레코드 경계가 장비·버전마다 다르다). 평문은 **뒤 후보로 남긴다** — CSV 미지원 항목 대비.
      `unityCsvOrder2529.test.js` 가 '첫 후보는 CSV' 를 전 항목에 대해 고정한다.
      **교훈**: 실장비 캡처가 평문이라고 해서 **파서가 평문을 더 잘 읽는다는 뜻이 아니다** —
      사람이 보기 좋은 형식과 기계가 읽기 좋은 형식은 다르다.
