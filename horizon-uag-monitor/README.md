...
- **세계지도 '글로벌 사이트 현황'**: 대상(사이트)을 위경도로 지도에 상태 색상 점으로 표시.
  **도시명 입력 → 좌표 자동(오프라인)**, **내 위치(사용자) 지정** 시 사용자→사이트 arc와
  사이트별 RTT를 지도에 표시. 지도/카드/표 3가지 보기 전환.
- **표 정렬**: 표 보기에서 컬럼 제목 클릭으로 정렬(다시 클릭 시 오름/내림 토글), 자동 새로고침에도 유지.
